...
export class EditorToolbar {
    constructor() {
        ...
        this.toolbar.addEventListener("mousedown", (e) => {
            ...
            if (heading !== undefined) {
                ...
                const blockType = (node.attrs.level === args.level)
    ...
    #initalizeElements() {
        ...
        const mappingFn = (key, iconPrefix = '') => ( (v) => {
    ...
    updateActiveState() {
        ...
        const hasMark = (type) => marks.some( mark =>
            mark.type === state.schema.marks[type]
        );

        const matchesNode = (type, value) => 
            node.attrs[type] === value;

        const toggleClass = (selector, condition) =>
            this.toolbar.querySelector(selector)
